"""CLI Script to Train Clean Baseline VGG-16 on CIFAR-10."""
...
def train_vgg16(epochs: int = 100, batch_size: int = 128, lr: float = 0.05, seed: int = 42):
    ...
    ckpt_dir = Path(resolved_paths.paths.checkpoints.base_models) / "vgg16"

    set_seed(seed)
    device = get_device()

    print("=" * 65)
    print("      VulnShield-DNN: Train Clean VGG-16 on CIFAR-10")
    print("=" * 65)
    ...
    # 2. Instantiate Model
    model = create_model("vgg16", num_classes=10, device=device)
    ...
    results = trainer.fit(
        ...
        checkpoint_name="vgg16_clean"
    )

    # 5. Evaluate on Test Set
    test_res = evaluate_model(model, loaders.test, device=device)
    print(f"\n[FINAL CLEAN TEST EVALUATION] VGG-16 Clean Test Accuracy: {test_res.accuracy:.2f}% (Loss: {test_res.loss:.4f})")
    print("=" * 65)


def main():
    parser = argparse.ArgumentParser(description="Train Clean VGG-16 on CIFAR-10")
    ...
    train_vgg16(epochs=args.epochs, batch_size=args.batch_size, lr=args.lr, seed=args.seed)
